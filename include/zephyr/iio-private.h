...
//#include "iio-config.h" //exists but is .cmakein ?
...
struct iio_attr_list {
	void * iio_dev_attr; //TODO needs to be tied to zephyr attribute struct
...
size_t iio_snprintf_channel_xml(char *str, size_t slen,
				 const struct iio_channel *chn);
size_t iio_snprintf_device_xml(char *str, size_t slen,
				const struct iio_device *dev);
...
size_t iio_xml_print_and_sanitized_param(char *ptr, size_t len,
					  const char *before,
					  const char *param,
					  const char *after);

static inline void iio_update_xml_indexes(size_t ret, char **ptr, size_t *len,
					  size_t *alen)
{
	if (*ptr) {
		*ptr += ret;
