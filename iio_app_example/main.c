...
// 	if (!(expr)) { \
// 		(void) fprintk(stderr, "assertion failed (%s:%d)\n", __FILE__, __LINE__); \
// 		(void) abort(); \
// 	} \
// }
...
{
	printk("Waiting for process to finish... Got signal %d\n", sig);
	//stop_stream();
}

/* check return value of attr_write function */
static void errchk(int v, const char* what) {
	 //if (v < 0) { fprintk(stderr, "Error %d writing to channel \"%s\"\nvalue may not be supported.\n", v, what); shutdown(); }
}
...
{
	snprintk(tmpstr, sizeof(tmpstr), "%s%d", type, id);
	return tmpstr;
}
...
bool cfg_ad9361_streaming_ch(struct stream_cfg *cfg, enum iodev type, int chid)
{
	...
	printk("* Acquiring AD9361 phy channel %d\n", chid);
	if (!get_phy_chan(type, chid, &chn)) {	return false; } // main.c

	attr = iio_channel_find_attr(chn, "rf_port_select"); //channel.c
	if (attr)
		errchk(iio_attr_write_string(attr, cfg->rfport), cfg->rfport);
	wr_ch_lli(chn, "rf_bandwidth",       cfg->bw_hz);
	wr_ch_lli(chn, "sampling_frequency", cfg->fs_hz);

	// Configure LOcaloscilator channel
	// printk("* Acquiring AD9361 %s lo channel\n", type == TX ? "TX" : "RX");
...
int main (void)
{
	...
	printk("* Acquiring IIO context\n");
	// if (argc == 1) {
		ctx = iio_create_context(NULL, NULL); //context.c
		err = iio_err(ctx); // iio.h inline
	...
	err = iio_context_get_devices_count(ctx);
	printk("* Acquiring AD9361 streaming devices\n");
	//IIO_ENSURE(get_ad9361_stream_dev(TX, &tx) && "No tx dev found");
	//IIO_ENSURE(get_ad9361_stream_dev(RX, &rx) && "No rx dev found");
	get_ad9361_stream_dev(TX, &tx);
	//get_ad9361_stream_dev(RX, &rx);
	printk("* Configuring AD9361 for streaming\n");
	...
	cfg_ad9361_streaming_ch(&txcfg, TX, 0); //TODO proveriti dal je nasao chn
	printk("* Initializing AD9361 IIO streaming channels\n");
	...
	bool i;
	i = get_ad9361_stream_ch(TX, tx, 0, &tx0_i);
	printk("%d", i);
	// rxmask = iio_create_channels_mask(iio_device_get_channels_count(rx));//mask.c
	// if (!rxmask) {
	// 	//fprintk(stderr, "Unable to alloc channels mask\n");
	// 	shutdown();
	// }
	...
	printk("* Enabling IIO streaming channels\n");
	// iio_channel_enable(rx0_i, rxmask); //channel.c
	// iio_channel_enable(rx0_q, rxmask);
	iio_channel_enable(tx0_i, txmask);
	// iio_channel_enable(tx0_q, txmask);

	printk("* Creating non-cyclic IIO buffers with 1 MiS\n");
	...
	printk("* Starting IO streaming (press CTRL+C to cancel)\n");
	// stream(rx_sample_sz, tx_sample_sz, BLOCK_SIZE,
	//       rxstream, txstream, rx0_i, tx0_i); // iostream-common.c
	iio_backend_read(tx->zephyr_dev, NULL, NULL);
	shutdown();
